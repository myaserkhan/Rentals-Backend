  ...
  path '/api/v1/cars' do
    post 'New car' do
      ...
      parameter name: :car, in: :body, schema: {
        type: :object,
        properties: {
          name: { type: :string }, description: { type: :string }, price: { type: :number, format: :float },
          duration: { type: :integer }, imgurl: { type: :string }, brand: { type: :string }
        },
        required: %w[name description price duration imgurl brand]
      }

      response '201', 'Car created successfully' do
        let(:car) do
          {
            name: 'Mehran', description: 'Affordable car', price: 1500, duration: 3,
